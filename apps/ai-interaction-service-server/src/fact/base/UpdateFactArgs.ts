/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { FactWhereUniqueInput } from "./FactWhereUniqueInput";
import { ValidateNested } from "class-validator";
import { Type } from "class-transformer";
import { FactUpdateInput } from "./FactUpdateInput";

@ArgsType()
class UpdateFactArgs {
  @ApiProperty({
    required: true,
    type: () => FactWhereUniqueInput,
  })
  @ValidateNested()
  @Type(() => FactWhereUniqueInput)
  @Field(() => FactWhereUniqueInput, { nullable: false })
  where!: FactWhereUniqueInput;

  @ApiProperty({
    required: true,
    type: () => FactUpdateInput,
  })
  @ValidateNested()
  @Type(() => FactUpdateInput)
  @Field(() => FactUpdateInput, { nullable: false })
  data!: FactUpdateInput;
}

export { UpdateFactArgs as UpdateFactArgs };
